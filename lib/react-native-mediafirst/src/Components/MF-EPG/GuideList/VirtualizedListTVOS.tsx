...
import ScrollView from "../../ScrollViewTVOS/CustomScrollView";
import { ReactElement } from "react";

import { DangerouslyImpreciseStyleProp, ViewStyleProp } from "StyleSheet";
...
export interface renderItemType {
    info: (info: any) => ReactElement<any>;
}

interface ViewabilityHelperCallbackTuple {
    viewabilityHelper: { recordInteraction: () => void, dispose: () => void};
...
interface RequiredProps {
    ...
    renderItem: (info: any) => ReactElement<any>;
    /**
...
interface OptionalProps {
    ...
    keyExtractor: (items: Item | Array<Item>, index: number) => string;
    /**
     ...
     * a rendered element.
     */
    ListEmptyComponent?: React.ComponentType<any> | ReactElement<any>;
    /**
     * Rendered at the bottom of all the items. Can be a React Component Class, a render function, or
     * a rendered element.
     */
    ListFooterComponent?: React.ComponentType<any> | ReactElement<any>;
    /**
     ...
     * a rendered element.
     */
    ListHeaderComponent?: React.ComponentType<any> | ReactElement<any>;
    /**
 ...
 *
 */
class VirtualizedListTVOS extends React.PureComponent<Props, State> {
    ...
    scrollToIndex(params: { animated?: boolean; index: number; viewOffset?: number; viewPosition?: number }) {
        ...
        if (!getItemLayout && index > this._highestMeasuredFrameIndex) {
            ...
            onScrollToIndexFailed !== undefined && onScrollToIndexFailed({
        ...
        const offset =
            Math.max(0, frame.offset - (viewPosition || 0) * (this._scrollMetrics.visibleLength - frame.length)) -
            (viewOffset || 0);
        /* $FlowFixMe(>=0.53.0 site=react_native_fb,react_native_oss) This comment
         * suppresses an error when upgrading Flow's support for React. To see the
         * error delete this comment and run Flow. */
    ...
    _unregisterAsNestedChild = (childList: { key: string; state: ChildListState }): void => {
        this._nestedChildLists.set(childList.key, {
            ref: null,
    ...
    constructor(props: Props, context: Object) {
        ...
        if (this.props.viewabilityConfigCallbackPairs) {
            this._viewabilityTuples = this.props.viewabilityConfigCallbackPairs.map((pair: {viewabilityConfig: any, onViewableItemsChanged: any}) => ({
    ...
    render() {
        ...
        if (ListHeaderComponent) {
            ...
            cells.push(
                <VirtualizedCellWrapper cellKey={this._getCellKey() + "-header"} key="$header">
                    <View
        ...
        if (itemCount > 0) {
            ...
            if (!isVirtualizationDisabled && first > lastInitialIndex + 1) {
                ...
                if (stickyIndicesFromProps.size > 0) {
                    ...
                    for (let ii = firstAfterInitial - 1; ii > lastInitialIndex; ii--) {
                        if (stickyIndicesFromProps.has(ii + stickyOffset)) {
                            ...
                            cells.push(<View key="$sticky_lead" style={{ [spacerKey]: leadSpace }} />);
                            this._pushCells(cells, stickyHeaderIndices, stickyIndicesFromProps, ii, ii, inversionStyle);
                            const trailSpace =
                                this._getFrameMetricsApprox(first).offset - (stickyBlock.offset + stickyBlock.length);
                            cells.push(<View key="$sticky_trail" style={{ [spacerKey]: trailSpace }} />);
                ...
                if (!insertedStickySpacer) {
                    const initBlock = this._getFrameMetricsApprox(lastInitialIndex);
                    const firstSpace =
                        this._getFrameMetricsApprox(first).offset - (initBlock.offset + initBlock.length);
                    cells.push(<View key="$lead_spacer" style={{ [spacerKey]: firstSpace }} />);
                }
            }
            this._pushCells(
                cells,
            ...
            if (!isVirtualizationDisabled && last < itemCount - 1) {
                ...
                cells.push(<View key="$tail_spacer" style={{ [spacerKey]: tailSpacerLength }} />);
            }
        } else if (ListEmptyComponent) {
            const element: ReactElement<any> = React.isValidElement(ListEmptyComponent) ? (
                ListEmptyComponent
            ) : (
                // $FlowFixMe
                <ListEmptyComponent />
        ...
        if (ListFooterComponent) {
            ...
            cells.push(
                <VirtualizedCellWrapper cellKey={this._getCellKey() + "-footer"} key="$footer">
                    <View
        ...
        const scrollProps = {
            ...
            invertStickyHeaders:
                this.props.invertStickyHeaders !== undefined ? this.props.invertStickyHeaders : this.props.inverted,
            stickyHeaderIndices,
        };
        ...
        // const ret = <View ref={this._captureScrollRef}>{cells} </View>;
        if (this.props.debug) {
            return (
                <View style={{ flex: 1 }}>
                    {ret}
    ...
    _nestedChildLists: Map<string, { ref: VirtualizedListTVOS | null; state: ChildListState | null }> = new Map();
    ...
    _selectLength(metrics: { readonly height: number; readonly width: number }): number {
        return !this.props.horizontal ? metrics.height : metrics.width;
    }

    _selectOffset(metrics: { readonly x: number; readonly y: number }): number {
    ...
    _maybeCallOnEndReached() {
        ...
        if (
            ...
            (this._hasDataChangedSinceEndReached || this._scrollMetrics.contentLength !== this._sentEndForContentLength)
        ) {
     ...
     */
    _convertParentScrollMetrics = (metrics: { visibleLength: number; offset: number }) => {
    ...
    _getFrameMetrics = (
        ...
        const item: Item = getItem(data, index);
        ...
        if (!CellRendererComponent) {
            return (
                <TouchableWithoutFeedback
                    ...
                    onBlur={() => {
                        // this.setState({ isFocused: false }) /* , this.state.isFocused ? { zIndex: 1 } : null */
                    }}
                    style={[cellStyle]}
                    onLayout={onLayout}
                >
...
export default VirtualizedListTVOS;